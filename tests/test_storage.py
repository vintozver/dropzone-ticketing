...
class StorageTicketDocumentTest(unittest.TestCase):
    ...
    def test_fido2_credential_embedded_document_fields(self) -> None:
        ...
        self.assertNotIn("attestation_object", fields)
        self.assertIsInstance(fields["attestation_aaguid"], mongoengine.BinaryField)
        self.assertFalse(fields["attestation_aaguid"].required)
        self.assertIsInstance(fields["attestation_publickey"], mongoengine.BinaryField)
        self.assertFalse(fields["attestation_publickey"].required)
        self.assertIsInstance(fields["extensions"], mongoengine.DictField)
        self.assertFalse(fields["extensions"].required)
